...
## Adding componets to streamlit for side by side comparison of vendors 

st.header("📊 Vendor Comparison Matrix")
selected_vendors = st.multiselect("Select Vendors to Compare:", search_results['vendor_name'].unique())

if selected_vendors:
    comparison_df = search_results[search_results['vendor_name'].isin(selected_vendors)]
    st.table(comparison_df[['vendor_name', 'current_price', 'sustainability_score', 'certification_status']])